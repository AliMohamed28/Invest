...
{
    public partial class backtestsma_stocks : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (Session["EMAILID"] != null)
            {
                ...
                this.Title = "BackTest Graph with Forecast";
        ...
        public void ShowBackTestGraph()
        {
            ...
            try
            {
                DataTable dailysmaTable = stockManager.GetBacktestFromSMA(symbol, exchange, "CLOSE", "Full", "1d",
        ...
        public void ShowSymbolValuation()
        {
            ...
            if ((valuationTable != null) && (valuationTable.Rows.Count > 0))
            {
                ...
                if (chartBackTest.Series.FindByName(symbol) == null)
                {
                    ...
                    chartBackTest.Series[symbol].ChartType = SeriesChartType.Line;
                ...
                {
                    chartBackTest.Series[symbol].Enabled = true;
                }
                chartBackTest.Series[symbol].LegendText = symbol;//valuationTable.Rows[0]["SYMBOL"].ToString() + "." + valuationTable.Rows[0]["EXCHANGE"].ToString();
                chartBackTest.Series[symbol].LegendToolTip = symbol;// valuationTable.Rows[0]["SYMBOL"].ToString();
                ...
                for (int rownum = 0; rownum < valuationTable.Rows.Count; rownum++)
                {
                    ...
                    if ((valuationTable.Rows[rownum]["PORTFOLIO_FLAG"].Equals("True")) || ((rownum + 1) == valuationTable.Rows.Count))
                    {
                        ...
                        chartBackTest.Series[symbol].Points[chartBackTest.Series[symbol].Points.Count - 1].MarkerStyle = MarkerStyle.Diamond;
