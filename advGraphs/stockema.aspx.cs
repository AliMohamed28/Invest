...
{
    public partial class stockema : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (Session["EMAILID"] != null)
            {
                ...
                if (!IsPostBack)
                {
                    ...
                    if (Request.QueryString["smallperiod"] != null)
                    {
                        Master.textboxSMADailyFastPeriod.Text = Request.QueryString["smallperiod"].ToString();
                    }
                    if (Request.QueryString["longperiod"] != null)
                    {
                        Master.textboxSMADailySlowPeriod.Text = Request.QueryString["longperiod"].ToString();
                    }
        ...
        public void ManagePanels()
        {
            ...
            Master.panelSMADaily.Enabled = true;
            Master.panelSMADaily.Visible = true;
        }
        ...
        public void fillGraphList()
        {
            ...
            Master.dropdownGraphList.Items.Add(li);
            li = new ListItem("EMA Fast", "EMA_SMALL");
            Master.dropdownGraphList.Items.Add(li);

            li = new ListItem("EMA Slow", "EMA_LONG");
            Master.dropdownGraphList.Items.Add(li);
        ...
        public void ShowDaily()
        {
            ...
            int smallPeriod = Int32.Parse(Master.textboxSMADailyFastPeriod.Text);
            int longPeriod = Int32.Parse(Master.textboxSMADailySlowPeriod.Text);
            try
            {
                dailyData = stockManager.GetSMA_EMA_MACD_BBANDS_Table(symbol, exchange, seriestype, outputsize, interval, fromDate, smallPeriod, longPeriod,
                    emaRequired: true);
        ...
        public void ShowEMA()
        {
            string smallperiod = Master.textboxSMADailyFastPeriod.Text;
            string longperiod = Master.textboxSMADailySlowPeriod.Text;
            string symbol = Master.textbox_SelectedSymbol.Text;
            string exchange = Master.textbox_SelectedExchange.Text;
            ...
            try
            {
                ...
                if ((smallperiod.Equals(string.Empty) == false) && (Int32.Parse(smallperiod) > 0))
                {
                    //chartAdvGraph.DataManipulator.FinancialFormula(FinancialFormula.MovingAverage, smallperiod, "OHLC:Y4", "SMA_SMALL:Y");
                    chartAdvGraph.Series["EMA_SMALL"].PostBackValue = "EMA Fast," + symbol + "." + exchange + "," + "#VALX,#VALY{0.##}";
                    chartAdvGraph.Series["EMA_SMALL"].Enabled = true;
                }
                if ((longperiod.Equals(string.Empty) == false) && (Int32.Parse(longperiod) > 0))
                {
                    //hartAdvGraph.DataManipulator.FinancialFormula(FinancialFormula.MovingAverage, longperiod, "OHLC:Y4", "SMA_LONG:Y");
                    chartAdvGraph.Series["EMA_LONG"].PostBackValue = "EMA Slow," + symbol + "," + "#VALX,#VALY{0.##}";
                    chartAdvGraph.Series["EMA_LONG"].Enabled = true;
                }
            }
            catch (Exception ex)
            {
