...
{
    public partial class buysellindicator : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (Session["EMAILID"] != null)
            {
                ...
                this.Title = "Buy Sell Indicator Graph";
        ...
        public void buttonShowSelectedIndicatorGraph_Click()
        {
            ...
            for (int i = 0; i < Master.dropdownGraphList.Items.Count; i++)
            {
                    ...
                    (chartAdvGraph.Series[Master.dropdownGraphList.Items[i].Value].Enabled))
                {
                    ...
                    else if (Master.dropdownGraphList.Items[i].Value.Equals("K-FastLine") || Master.dropdownGraphList.Items[i].Value.Equals("D-SlowLine"))
                    {
                        //all other graphs are on area 0
                        bArea1 = true;
                    }
                    else if (Master.dropdownGraphList.Items[i].Value.Equals("RSI"))
                    {
                        //all other graphs are on area 0
                        bArea2 = true;
                    }
